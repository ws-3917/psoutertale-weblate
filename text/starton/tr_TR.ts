...
export default {
    a_starton: {
        telescope1: () => [
            ...
            choicer.create('* (Teleskobu kullan?)', 'Evet', 'Hayır')
        ],
        ...
        telescopeMeetup3: [
            ...
            '<25>{#f/1}* O kısa iskelet bana bir süre önce bunlardan bir sürü vermişti.',
            '<25>{#f/2}* Ayrıca içinde çok para içeren dijital bir şey verdi...',
            '<25>{#f/1}* Sanırım beni gerçekten seviyor, haha.'
        ],
        telescopeMeetup4: [
            '<25>{#p/kidd}{#f/3}* Anyway, I kinda just wanted to give you the voucher.',
            '<25>{#f/1}* Ama umarım bununla havalı bir şey görürsün!'
        ],
        telescopeMeetup5: ["<25>{#p/kidd}{#f/1}* Ben kasabada olacağım!"],
        telescope2: () =>
            SAVE.data.b.svr
                ? ['<25>{#p/asriel1}{#f/17}* Beğendiğin bir şey görüyor musun?']
                : SAVE.data.b.oops || SAVE.data.b.s_state_chargazer
                    ? ['<32>{#p/basic}* Uzaydan yıldızlara bakmak...\n* Gerçekten de, bu alışılmışın dışında bir düşünce.']
                    : ((SAVE.data.b.s_state_chargazer = true),
                        [
                            '<32>{#p/basic}* ...',
                            '<32>* Asriel ve benim de tıpkı bunun gibi bir teleskobumuz vardı.',
                            "<32>* Heyecan verici bir şey görme umuduyla rastgele yönlere doğrulturduk...",
                            '<32>* ... hiçbir zaman çok bir şey göremedik.',
                            "<32>* Buna rağmen, onun için bu pek de önemli değildi...",
                            '<32>* Ben dışarıya bakmaya odaklanmışken, o sadece benimle vakit geçirmekten mutluydu.',
                            '<32>* ...',
                            '<32>{#p/human}* (Bir iç çekiş duyuyorsun.)',
                            "<32>{#p/basic}* ... eh, hadi en son ne yapıyorsak ona geri dönelim."
                        ]),
        notv: ["<32>{#p/basic}* Gözlemleyecek ilginç bir şey varmış gibi görünmüyor."],
        nicecreamScoreReaction1a: ['<32>{#p/basic}* İlk denemene göre fena değil...'],
        nicecreamScoreReaction1b: ['<32>{#p/basic}* İlk denemene göre fena değil.'],
        nicecreamScoreReaction2a: ['<32>{#p/basic}* Bundan daha iyisini yapabilirsin...'],
        nicecreamScoreReaction2b: ['<32>{#p/basic}* Bundan daha iyisini yapabilirsin.'],
        ...
        nicecreamScoreReaction4a: ['<33>{#p/basic}* Bunda oldukça iyi görünüyorsun...'],
        nicecreamScoreReaction4b: ['<32>{#p/basic}* Bunda oldukça iyi görünüyorsun.'],
        ...
        nicecreamScoreReaction7a: ['<32>{#p/basic}* Biraz pratik yapman gerekiyor gibi görünüyor...'],
        nicecreamScoreReaction7b: ['<32>{#p/basic}* Biraz pratik yapman gerekiyor gibi görünüyor.'],
        nicecreamScoreReaction8a: ["<32>{#p/basic}* Bu daha iyi..."],
        nicecreamScoreReaction8b: ["<32>{#p/basic}* Bu daha da olması gerektiği gibi."],
        ...
        nicecreamScoreReaction10a: ["<32>{#p/basic}* İlk denemeye göre, bu oldukça iyi..."],
        ...
        jukebox1: () => [
            SAVE.data.b.svr
                ? '<32>{#p/human}* (Müzik kutusuna uzanıyorsun...)'
                : "<32>{#p/basic}* Bu müzik kutusu yalnızca daha önce duyduğun şarkıları çalıyor.",
            choicer.create(
                '* (Bir şarkı çal?)',
                SAVE.data.b.napsta_performance ? 'Parça 01' : '???',
                2 <= SAVE.data.n.state_foundry_swansong ? 'Parça 02' : '???',
                2 <= SAVE.data.n.state_starton_trashprogress ? 'Parça 03' : '???',
                'Vazgeç'
            )
        ],
        jukebox1x1: () =>
            SAVE.data.b.svr
                ? ["<32>{#p/human}* (Ama henüz bilmediğin bir şarkıyı çalamadın.)"]
                : ["<32>{#p/basic}* Kapakta kalabalığa müzik çalan ürkütücü bir DJ görülüyor.\n* Bu şarkıyı bilemiyorsun."],
        jukebox1x2: () =>
            SAVE.data.b.svr
                ? ["<32>{#p/human}* (Ama henüz bilmediğin bir şarkıyı çalamadın.)"]
                : ["<33>{#p/basic}* Kapakta bilgisayarının başında bulunan ürkütücü bir DJ görülüyor.\n* Bu şarkıyı bilemiyorsun."],
        jukebox1x3: () =>
            SAVE.data.b.svr
                ? ["<32>{#p/human}* (Ama henüz bilmediğin bir şarkıyı çalamadın.)"]
                : [
                    "<32>{#p/basic}* Kapakta çöpler arasında bulunan küçük beyaz bir köpeği görülüyor.\n* Bu şarkıyı bilemiyorsun."
                ],
        jukebox1y: ['<32>{*}{#p/human}* (Diski seçiyorsun...){^40}{%}'],
        jukebox2: () => [
            SAVE.data.b.svr
                ? '<32>{#p/human}* (Şu anda bir şarkı çalıyor gibi görünüyor.)'
                : [
                    '<32>{#p/basic}* Şu anda \"Parça 01\" çalıyor',
                    '<32>{#p/basic}* Şu anda \"Parça 02\" çalıyor',
                    '<32>{#p/basic}* Şu anda \"Parça 03\" çalıyor'
                ][SAVE.data.n.state_starton_jukebox - 1],
            choicer.create('* (Oynatmayı durdur?)', 'Evet', 'Hayır')
        ],
        jukebox3a1: ["<32>{#p/basic}{#npc/a}* İşte bu daha da yakışır!"],
        jukebox3a2: ['<32>{#p/basic}{#npc/a}* (Biz bu tür müzikleri çok seviyoruz.)'],
        jukebox3b: ['<32>{#p/basic}{#npc/a}* Bu dans kulübünde dolaşıp duran şarkı mı?'],
        jukebox3c: [
            '<32>{#p/basic}* ...\n* ...\n* ...',
            "<32>{#npc/a}* Grillbz bu şarkıyı daha önce bir yerde duyduğunu söylüyor."
        ],
        jukebox3d: [
        ...
        shockpapyrus0a: [
            '<15>{#p/papyrus}{#e/papyrus/27}KRİOS AŞKINA NELER OLUYOR BURADA??',
            '<15>{#p/papyrus}{#e/papyrus/21}BALKONUMDAN BİR ARAMAYA YANIT VERİYORUM...',
            '<15>{#p/papyrus}{#e/papyrus/19}VE BENİ YÖNLENDİRDİĞİ YER BURASI MI!?',
            "<15>{#p/papyrus}{#e/papyrus/14}ŞUNU BELİRTMEK İSTERİM Kİ, KRALİYET MUHAFIZI İÇİN HAZIR BİR ADAYIM.",
            '<15>{#p/papyrus}{#e/papyrus/15}YANİ, SİZ İKİNİZ HER NE PLANLIYORSANIZ...'
        ],
        shockpapyrus0b: [
            '<15>{#p/papyrus}{#e/papyrus/24}... BEKLE, O SES...',
            '<15>{#p/papyrus}{#e/papyrus/22}BENİ BİR SÜRE ÖNCE TELEFONLA ARAYAN SEN MİYDİN!?'
        ],
        shockpapyrus0c: [
            '<15>{#p/papyrus}{#e/papyrus/20}... TABİİ Kİ!\nBU HER ŞEYİ AÇIKLAR!',
            "<15>{#p/papyrus}{#e/papyrus/10}PEKALA ÖYLEYSE.\nSENİNLE SONUNDA TANIŞTIĞIMA MEMNUN OLDUM.",
            '<15>{#p/papyrus}{#e/papyrus/24}DÜRÜST OLMAM GEREKİRSE, BANA ANIMSATTIĞIN BİRİ VAR...',
            '<15>{#p/papyrus}{#e/papyrus/20}... HEY, BEKLE BİR DAKİKA!!',
            '<15>{#p/papyrus}{#e/papyrus/22}YANINDA BİR -İNSAN- MI GETİRDİN!?!?',
            '<15>{#p/papyrus}{#e/papyrus/10}WOWIE!!\nBU GİTTİKÇE DAHA DA İYİ OLUYOR!!',
            '<15>{#p/papyrus}{#e/papyrus/20}ÖYLEYSE, BURADA NE YAPIYORUZ Kİ?'
        ],
        shockpapyrus1: () =>
            [
                [
        ...
        innkeep1b: pager.create(
            ...
            () => [
                ...
                choicer.create('* (Get a room again?)', 'Evet', 'Hayır')
            ],
            () => ['<32>{#p/basic}{#npc/a}* Changed your mind?', choicer.create('* (Get a room again?)', 'Evet', 'Hayır')]
        ),
            ...
            papmail1: () =>
                SAVE.data.b.svr
                    ...
                    : [
                        ...
                        choicer.create('* (Look inside the mailbox?)', 'Evet', 'Hayır')
                    ],
            ...
            () =>
                postSIGMA()
                    ...
                    : [
                        ...
                        choicer.create("* (Log in to Papyrus's account?)", 'Evet', 'Hayır')
                    ],
            () =>
                postSIGMA()
                    ? ["<32>{#p/basic}* Hizmet dışı."]
        ...
        papcouch1: pager.create(
            ...
            () => [
                ...
                choicer.create('* (Take the coins?)', 'Evet', 'Hayır')
            ],
            () => [
                SAVE.data.b.svr
        ...
        papdate30: () => [choicer.create('* (Do you know what it is?)', 'Evet', 'Hayır')],
        papdate31a: [
